...
Requires:
  - videoconv64: PATH, N64_INST/bin, libdragon/tools/videoconv64/, or env VIDEOCONV64=full path
  - ffmpeg / ffprobe on PATH (or set paths below)
  - audioconv64: same discovery as video2n64_gui (find_audioconv64); a temp N64_INST shim is used
    when the binary is not already under <prefix>/bin/ (videoconv64 only looks there).
...
from n64fmv_lib import find_audioconv64
...
def _normalize_n64_inst_input(raw: str) -> Path | None:
    ...
    if not s:
        return None
    p = Path(s).expanduser()
    if p.is_file() and p.name in ("audioconv64", "audioconv64.exe"):
        return _prefix_from_audioconv_path(p)
    if p.is_dir() and p.name == "bin" and (
        (p / "audioconv64").is_file() or (p / "audioconv64.exe").is_file()
    ):
        return p.parent
    if (
        p.is_dir()
        and p.name == "audioconv64"
        and p.parent.name == "tools"
        and (
            (p / "audioconv64").is_file()
            or (p / "audioconv64.exe").is_file()
        )
    ):
...
def _n64_inst_for_videoconv64(ac: Path) -> tuple[str, Path | None]:
    """videoconv64 only accepts $N64_INST/bin/audioconv64[.exe]; shim with a temp dir if needed."""
    if ac.parent.name == "bin" and _is_runnable(ac):
        root = ac.parent.parent.resolve()
        return str(root), None
    dest_name = "audioconv64.exe" if os.name == "nt" else "audioconv64"
    td = Path(tempfile.mkdtemp(prefix="n64video-vconv-n64inst-"))
    bindir = td / "bin"
    bindir.mkdir(parents=True, exist_ok=True)
    dest = bindir / dest_name
    try:
        os.symlink(ac.resolve(), dest, target_is_directory=False)
    except OSError:
        shutil.copy2(ac, dest)
        if os.name != "nt":
            mode = dest.stat().st_mode
            dest.chmod(mode | 0o111)
    return str(td.resolve()), td


def find_videoconv64(n64_inst: str | None) -> Path | None:
    """Resolve videoconv64: env, PATH, same bin/ as audioconv64, libdragon tree builds."""
    ev = os.environ.get("VIDEOCONV64", "").strip()
    if ev:
    ...
    n64p = Path(n64_inst).expanduser() if n64_inst else None
    ac = find_audioconv64(n64p)
    if not ac and os.environ.get("N64_INST", "").strip():
        ac = find_audioconv64(Path(os.environ["N64_INST"].strip()).expanduser())
    if not ac:
    ...
    for prefix in uniq:
        for name in ("videoconv64", "videoconv64.exe"):
            ...
            if _is_runnable(cand):
                return cand
        # Uninstalled preview build: `make tools` leaves a binary under tools/videoconv64/
        for name in ("videoconv64", "videoconv64.exe"):
            cand = prefix / "tools" / "videoconv64" / name
            if _is_runnable(cand):
                return cand
    return None
...
def _guess_n64_inst() -> str:
    ...
    if env:
        ...
        if n64p and (
            (n64p / "bin" / "audioconv64").is_file()
            or (n64p / "bin" / "audioconv64.exe").is_file()
        ):
            return str(n64p)
    ...
    for c in candidates:
        if (c / "bin" / "audioconv64").is_file() or (c / "bin" / "audioconv64.exe").is_file():
            return str(c)
        if (
            (c / "bin" / "videoconv64").is_file()
            or (c / "tools" / "videoconv64" / "videoconv64").is_file()
        ):
            return str(c)
    return ""
...
class App(tk.Tk):
    ...
    def _build(self) -> None:
        ...
        ttk.Label(f_out, text="videoconv64 (or set VIDEOCONV64 env)").grid(
            row=2, column=0, sticky=tk.W, pady=(4, 0)
        )
        self._vc_path = tk.StringVar(value="")
        ev_vc = os.environ.get("VIDEOCONV64", "").strip()
        if ev_vc:
            self._vc_path.set(ev_vc)
    ...
    def _start(self) -> None:
        ...
        if not vc or not vc.is_file():
            messagebox.showerror(
                "videoconv64 not found",
                "This tool needs videoconv64 from libdragon preview (installed next to audioconv64).\n\n"
                "Try:\n"
                "  • Set N64_INST to your install folder, click Auto-find, or Browse to the binary.\n"
                "  • Or set environment variable VIDEOCONV64 to the full path to videoconv64.\n"
                "  • In libdragon preview: run ./build.sh (installs tools into $N64_INST/bin).\n"
                "  • If you only built tools: check libdragon/tools/videoconv64/videoconv64\n\n"
                "Windows: use WSL/Linux build of videoconv64, or Browse to a copy in N64_INST\\bin.\n\n"
        ...
        n64_inst_raw = self._n64.get().strip()
        n64_norm = _normalize_n64_inst_input(n64_inst_raw)
        user_prefix_for_find = n64_norm if n64_norm else None

        env = os.environ.copy()
        shim_cleanup: Path | None = None

        if not self._noaud.get():
            ac_tool = find_audioconv64(user_prefix_for_find)
            if not ac_tool:
                messagebox.showerror(
                    "audioconv64 not found",
                    "videoconv64 needs audioconv64 for .wav64 output. None was found using the "
                    "same search as the main FMV GUI (this window’s N64_INST field, the environment "
                    "variable N64_INST, and common install or libdragon tree paths).\n\n"
                    "Build tools from libdragon preview or set/browse to your toolchain prefix.",
                )
                return
            n64_for_env, shim_cleanup = _n64_inst_for_videoconv64(ac_tool)
            env["N64_INST"] = n64_for_env
            env["PATH"] = str(Path(n64_for_env) / "bin") + os.pathsep + env.get("PATH", "")
        elif n64_inst_raw:
            p = Path(n64_inst_raw).expanduser()
            env["N64_INST"] = str(p)
            env["PATH"] = str(p / "bin") + os.pathsep + env.get("PATH", "")
        ...
        def run() -> None:
            try:
                ...
                if shim_cleanup is not None:
                    shutil.rmtree(shim_cleanup, ignore_errors=True)
